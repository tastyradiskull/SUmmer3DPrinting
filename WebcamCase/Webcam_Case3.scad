...
translate([21.9,-5,-6])cube([9,9,7],center=true);
//window for sensor.
...
radius=12.4;
pitch=0.7938;
//radius=12.91   Changed from SM1 to Cmount thread;
//pitch=0.635;
difference(){
    union(){
    cylinder(d1=39,d2=30,h=7,$fn=100);
    cylinder(d=41.95,h=2,$fn=100);
        //Outer cylinders giving width of holder. Decreased this from Grahams original.
    }
...
inner_thread(radius=radius,threads_per_mm=pitch,thread_length=7);
} //changed height from 7 to 9mm

//CODE ABOVE FOR OPTIC HOLDER MODULE

translate([70,0,0])Optic_Holder();
//Calls optic holder.
